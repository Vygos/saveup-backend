...
public interface TipoGanhoRepository extends JpaRepository<TipoGanho, Integer> {
    ...
    @Query(value = "SELECT TG.NOME FROM SAVEUP.TIPO_GANHO TG WHERE LOWER(TG.NOME) = LOWER(:nome) AND TG.EMAIL = :email " +
            "UNION " +
            "SELECT TG.NOME FROM SAVEUP.TIPO_GANHO TG WHERE LOWER(TG.NOME) = LOWER(:nome) AND TG.EMAIL IS NULL ",
            nativeQuery = true)
